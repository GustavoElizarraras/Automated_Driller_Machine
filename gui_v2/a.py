...
def get_rotated_coords(row, angle):
    name = row[0]
    coords_list = row[1:]
    if angle == 180:
        name = name[:-4] + "_3.jpg"
    ...
    for i in range(0, len(coords_list), 4):
        ...
        stacked_coords.append(int(x1))
        stacked_coords.append(int(y1))
        stacked_coords.append(int(x2))
        stacked_coords.append(int(y2))
...
with open("dataset/processed_locations/hand_picked_better.csv", "r") as r:
    ...
    for row in reader:
        for angle in [90,270,180]:
            rotated_coords = get_rotated_coords(row, angle)
            with open("writer_dummy.csv", "a") as w:
                writer = csv.writer(w)
                writer.writerow(rotated_coords)
